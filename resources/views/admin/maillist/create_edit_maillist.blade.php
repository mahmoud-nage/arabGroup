        ...
        <div class="card-content">
          <div class="card-body">
            ...
            @if($record->name == null)
                   {{-- @dd('create') --}}
                   @section('name')
                      {{__('اضافه قائمة العملاء')}}
                    @endsection
                    <form class="form-horizontal" id="quickForm" method="POST" action="{{route('maillist.store')}}" enctype="multipart/form-data">

                @else
                   {{-- @dd('update') --}}
                   @section('name')
                      {{__('تعديل قائمة العملاء')}}
                   @endsection
                    <form class="form-horizontal" id="quickForm" method="POST" action="{{route('maillist.update', $record->id)}}" enctype="multipart/form-data">
                @endif
            @csrf 
              <div class="row">
